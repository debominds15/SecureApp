...
public class MaterialLockView extends View {
    ...
    private boolean isTransparentPath = false;
    ...
    public void setPatternInvisible(Context context,boolean isPatternInvisible){

        if(isPatternInvisible) {
            Log.d("MaterialLockView","it is invisible");
            setClickable(false);
            isTransparentPath = true;
            TypedArray typedArray = context.obtainStyledAttributes(null, R.styleable.MaterialLockView);
            mTransparentColor = typedArray.getColor(R.styleable.MaterialLockView_LOCK_COLOR, Color.WHITE);
            mErrorColor = typedArray.getColor(R.styleable.MaterialLockView_WRONG_COLOR, Color.WHITE);
            mSuccessColor = typedArray.getColor(R.styleable.MaterialLockView_CORRECT_COLOR, Color.WHITE);
            mPathPaint.setColor(mTransparentColor);
            mPathPaint.setStyle(Paint.Style.FILL);
            mPathPaint.setStrokeJoin(Paint.Join.ROUND);
            mPathPaint.setStrokeCap(Paint.Cap.ROUND);

            int mPathWidth = dpToPx(0);
            mPathPaint.setStrokeWidth(mPathWidth);
        }
        else{
            isTransparentPath = false;
            Log.d("MaterialLockView","it is visible");
            TypedArray typedArray = context.obtainStyledAttributes(null, R.styleable.MaterialLockView);
            mTransparentColor = typedArray.getColor(R.styleable.MaterialLockView_LOCK_COLOR, Color.WHITE);
    ...
    private void addCellToPattern(Cell newCell) {
        ...
        if (!mInStealthMode && !isTransparentPath) {
